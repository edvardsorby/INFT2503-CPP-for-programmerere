...
public:
  ...
  void positionSphere(float x, float y) {
    btTransform transform;
    transform.setIdentity();
  ...
  void resetVelocity() {
    ...
    ground_sphere.body.setLinearVelocity(btVector3(0, 0, 0));
    ground_sphere.body.setAngularVelocity(btVector3(0, 0, 0));

    for (auto &cube : cubes) {
      cube.body.setLinearVelocity(btVector3(0, 0, 0));
      cube.body.setAngularVelocity(btVector3(0, 0, 0));
    }
  }
};
  ...
  void start() {
    ...
    while (running) {
      ...
      if (ImGui::Button("Restart game")) {
        ...
        world.positionSphere(horizontal_position, vertical_position);
      }
      if (ImGui::Button("Drop ball")) {

        world.falling_sphere.body.activate();
        paused = false;
      }

      if (ImGui::SliderFloat("Horizontal ball position", &horizontal_position, 0.8, 1.2)) {
      ...
      if (!paused) {
        ...
        world.dynamics.stepSimulation(std::chrono::duration<float>(time - last_time).count()); // Only let time pass if game is not paused
        last_time = time;
      }
